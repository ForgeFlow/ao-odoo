...
<body>
    ...
    %for picking in objects:
        <% setLang(picking.partner_delivery_id.lang) %>
        <div class="address">
            <table class="recipient">
                    %if picking.partner_delivery_id.parent_id:
                    <tr><td class="name">${picking.partner_delivery_id.parent_id.name or ''}</td></tr>
                    <tr><td>${picking.partner_id.title and picking.partner_id.title.name or ''} ${picking.partner_delivery_id.name }</td></tr>
                    <% address_lines = picking.partner_delivery_id.contact_address.split("\n")[1:] %>
                    %else:
                    <tr><td class="name">${picking.partner_id.title and picking.partner_delivery_id.title.name or ''} ${picking.partner_delivery_id.name }</td></tr>
                    <% address_lines = picking.partner_delivery_id.contact_address.split("\n") %>
                    %endif
