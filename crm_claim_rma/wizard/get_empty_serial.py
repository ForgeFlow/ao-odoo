...
class get_empty_serial(models.TransientModel):
    _name='get_empty_serial.wizard'
    _description='' # Get possible serial for this return based on claim partner, product, invoice

    
    @api.model
    def temp_display_results(self):
        print "=====> IN TEMP display"
        self.temp = ""
        print "partner_id : ",self.env['crm.claim'].read(self._context['active_id'], ['partner_id'])['partner_id']
...
#        'prodlot_ids': fields.many2one('stock.production.lot', 'Serial / Lot Number 1', required=True),
    temp = fields.Text(string='Resultats', default=temp_display_results) 
